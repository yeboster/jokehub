...
const GenerateJokeInputSchema = z.object({
  ...
  model: z.enum(['googleai/gemini-2.0-flash', 'googleai/gemini-1.5-pro']).optional().describe('The model to use for generation.'),
...
const generateJokeFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    const res = await ai.generate({
      prompt,
      model: input.model || 'googleai/gemini-2.0-flash', // Default to flash if not provided
